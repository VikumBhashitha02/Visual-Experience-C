 ...
 * Fixed to remove missing C.lava property.
 */

import { C, drawConceptPill, drawRock, drawVolcano } from "../core/shapes";
...
export function drawBackground(ctx: Ctx, W: number, H: number): void {
  ...
  ctx.fillStyle = "#BF360C"; // lava layer (was C.lava which doesn't exist)
...
export function drawAnchorCharacters(
  ...
  drawVolcano(ctx, W * 0.72, H * 0.52, 52, 0.85);
  drawRock(ctx, W * 0.26, H * 0.56, 28, 0.9, "#A1887F");
  drawRock(ctx, W * 0.42, H * 0.61, 20, 0.75, "#78909C");
}
...
export function keywordFallback(
  ...
  const cx = W * 0.62;
  const cy = H * 0.38;
  drawConceptPill(ctx, cx, cy, fadeIn(elapsed, 300, 700), "#BF360C", "Rock Cycle");
}
