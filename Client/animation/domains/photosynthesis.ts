/**
 * domains/photosynthesis.ts — Background, anchors, and keyword fallback for photosynthesis.
 */
...
export function drawBackground(ctx: Ctx, W: number, H: number): void {
  ...
  drawCloud(ctx, W * 0.12, H * 0.09, 1.0, 0.75);
  drawCloud(ctx, W * 0.72, H * 0.07, 0.75, 0.65);
}
...
export function keywordFallback(
  ...
  if (/chemical|energy|store/.test(txt)) {
    ...
    drawBolt(ctx, cx - 40, cy, 32, a, C.bolt);
    const a2 = fadeIn(elapsed, 800, 600);
    if (a2 > 0) {
      drawArrow(ctx, cx - 2, cy, 0, 52, C.arrow, 2.5, a2);
      drawGlucose(ctx, cx + 95, cy, 34, a2, t, C.glucose);
    }
    return;
  }
  ...
  if (/inside|chloroplast|cell|leaf/.test(txt)) {
    const a = fadeIn(elapsed, 200, 700);
    const puls = 0.9 + Math.sin(t * 1.5) * 0.08;
    ctx.save();
    ...
    ctx.scale(1.55, 1);
    ctx.beginPath();
    ctx.arc(0, 0, 44 * puls, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    ctx.save();
    ctx.globalAlpha = a;
    ...
    ctx.restore();
    ctx.restore();
    drawBolt(ctx, W * 0.3, H * 0.35, 20, fadeIn(elapsed, 900, 500), C.bolt);
    return;
  }
  ...
  if (/oxygen|o2/.test(txt)) {
    [0, 320, 640].forEach((delay, i) => {
      const travel = easeOut(clamp01((elapsed - delay) / 2200));
      const bx = lerp(W * 0.4, W * 0.72 + i * 12, travel);
      const by = H * 0.26 + i * 36;
      drawO2(ctx, bx, by, 24, fadeIn(elapsed, delay, 400));
    });
    return;
  }
  ...
  // Default: animated light rays toward the leaf
